...
void mode(int freq[], const int answer[]) {
   ...
   for (size_t rating = 1; rating <= 9; ++rating) {
    ...
    if (multimodal){ 
        ...
        "For this run the mode is multimodal, so the modes are"); 
        for(size_t i = 0; i<10;i++){
            if (freq[i] == largest){
                printf("%d, ",freq[i]);
            }
        ...
        puts("\nThe mode is the most frequent value.\n"
        "For this run the mode is bimodal, so the modes are");
        for(size_t i = 0; i<10;i++){
            if (freq[i] == largest){
                printf("%d, ",freq[i]);
            }
